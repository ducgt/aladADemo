...
//---
   ...
   cx=(rc.Width()-::GetSystemMetrics(SM_CXVSCROLL)-::GetSystemMetrics(SM_CXBORDER)*5)/5;
   ...
   m_Users.InsertColumn(4, "Equity", LVCFMT_RIGHT, cx);
//---
   return(TRUE);
  }
//+------------------------------------------------------------------+
...
LRESULT CPageMargin::OnPumpingMsg(WPARAM wParam,LPARAM)
	{
		if(wParam==PUMP_UPDATE_TRADES || wParam==PUMP_UPDATE_MARGINCALL || wParam == PUMP_UPDATE_BIDASK)
		{
			if(m_levels!=NULL)
			{
			...
			//char aa[64];
			//_snprintf(aa, sizeof(aa) - 1, "%d", m_levels_total);

			//MessageBox(aa, "m_levels_total");
		}
		return(0);
	}
//+------------------------------------------------------------------+
      ...
      switch(pDI->item.iSubItem)
        {
		 ...
		 case 4: _snprintf(tmp, sizeof(tmp) - 1, "%.02lf", m_levels[i].equity);  break;
         case 3:
