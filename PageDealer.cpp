...
//+------------------------------------------------------------------+
//| По номеру даем описание команд?                                 |
//+------------------------------------------------------------------+
...
//---
   m_Server.SetWindowText("192.168.9.70:444");
   SetDlgItemInt(m_Login.GetDlgCtrlID(),1,FALSE);
   m_Password.SetWindowText("1qazse4");
//---
   EnableAnswer(FALSE);
//---
   return(TRUE);
...
void CPageDealer::OnConfirm()
  {
   char* comm = "test comment";
   memcpy(m_req.trade.comment,comm,sizeof(m_req.trade.comment)-1);
   //---
