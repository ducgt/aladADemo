...
//---
   ...
   m_Mails.InsertColumn(3,"Body",   LVCFMT_LEFT,cx*4);
//---
   return(TRUE);
  }
//+------------------------------------------------------------------+
...
//---
   if(wParam==PUMP_UPDATE_MAIL && ExtManagerPump->MailLast(path,&len)==RET_OK)
     {
//--- read full MailBox structure
         if((in=fopen(path,"rb"))!=NULL && fread(&mail,sizeof(mail),1,in)==1)
           {
            if((ptm=gmtime(&mail.time))==NULL)
		      {
			    COPY_STR(tmp,"unknown");
		      }
            else
		      {
			    _snprintf(tmp,sizeof(tmp)-1,"%04d.%02d.%02d %02d:%02d",1900+ptm->tm_year,1+ptm->tm_mon,ptm->tm_mday,ptm->tm_hour,ptm->tm_min);
		      }
	       }
	     if(in!=NULL) { fclose(in); in=NULL; }
//--- read mail body
		 if((in_body=fopen(path,"rb"))!=NULL)
		   {
			 char *body=new char[mail.bodylen+10];
			 if(body!=NULL)
			 {

				 if(fseek(in_body,sizeof(MailBox),SEEK_SET)!=0) ret = 1; 
				 if(fread(body,mail.bodylen,1,in_body)!=1)      ret = 1; 
                 if(ret==0) body[mail.bodylen]=0;
				 //---
				 m_Mails.InsertItem(0,tmp);
				 m_Mails.SetItemText(0,1,mail.from);
				 m_Mails.SetItemText(0,2,mail.subject);
				 //---- is it HTML message?
				 if(mail.bodylen>5 && _strnicmp(body,"<html",5)==0) // try to find "<html" tag in body
				 {
					 ReadHtmlBody(body);
				 }
				 else
				 {
				 //---- text only
					 m_Mails.SetItemText(0,3,body);
				 }
			 }
		    fclose(in_body); in_body=NULL;
			delete[] body;
		   }
     }
//---
...
//|                                                                  |
//+------------------------------------------------------------------+
void CPageMailbox::ReadHtmlBody(LPCSTR html_source)
{
