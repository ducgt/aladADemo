...
struct RequestInfo
  {
   ...
   void* __ptr32     trade_group;         // (internal use)
   int               reserved[2];         // reserved
   //---
...
//--- request logs type
enum EnLogType     { LOG_TYPE_STANDARD,      // all except logins
   ...
   inline void Init(LPCSTR lib_path=NULL)
     {
      ...
      if(lib_path!=NULL) 
        {
         strncpy(path,lib_path,sizeof(path)-1);
         path[sizeof(path)-1]=0;
        }
      else               
        { 
         #ifndef _WIN64
         strncpy(path,"mtmanapi.dll",sizeof(path)-1); 
         path[sizeof(path)-1]=0; 
         #else
         strncpy(path,"mtmanapi64.dll",sizeof(path)-1); 
         path[sizeof(path)-1]=0; 
         #endif
        }
      //---
